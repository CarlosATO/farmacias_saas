...
CREATE TABLE pharmacy.prescriptions (
  ...
  status text DEFAULT 'PENDING'::text CHECK (status = ANY (ARRAY['PENDING'::text, 'PARTIAL'::text, 'DISPENSED'::text, 'EXPIRED'::text, 'CANCELLED'::text])),
  created_at timestamp with time zone DEFAULT now(),
  created_by uuid,
  prescription_type text DEFAULT 'RECETA_SIMPLE'::text CHECK (prescription_type = ANY (ARRAY['RECETA_SIMPLE'::text, 'RECETA_RETENIDA'::text, 'RECETA_CHEQUE'::text])),
